//! C header parser for Τομί plugins.
//!
//! Parses C function declarations annotated with `// @tomii_export` and
...
/// Parsed attributes from a `// @tomii_export(...)` annotation line.
...
/// Parse the optional `(...)` body from `// @tomii_export(...)`.
///
...
/// Parse C header source text and return all `@tomii_export`-annotated
/// function entries.
pub(crate) fn collect_c_entries(source: &str) -> Vec<ExportedFn> {
    ...
    while i < lines.len() {
        ...
        if let Some(after_at) = line.strip_prefix("// @tomii_export") {
