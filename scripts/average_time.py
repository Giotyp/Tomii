...
def average_csv_analyses(
    csv_files: List[str], system_threads: int = 0
) -> Dict[str, Any]:
    """
...
def write_csv_analysis(metrics: Dict[str, Any], output_path: str, units: str = "us"):
    ...
    with open(output_path, "w") as f:
        ...
        for worker_id in sorted(metrics["workers"].keys()):
            ...
            f.write(
                f"  Total span:        {span_scaled:15.2f} {unit_label}  [100.00%]\n"
            )
            f.write(
                f"  Busy time:         {busy_scaled:15.2f} {unit_label}  [{busy_percent:6.2f}%]\n"
            )
            f.write(
                f"  Idle time:         {idle_scaled:15.2f} {unit_label}  [{idle_percent:6.2f}%]\n"
            )
            f.write("\n")
        ...
        avg_idle = (
            total_idle_all / metrics["num_workers"] if metrics["num_workers"] > 0 else 0
        )
        avg_busy = (
            total_busy_all / metrics["num_workers"] if metrics["num_workers"] > 0 else 0
        )
        avg_idle_percent = (
            (avg_idle / total_span_all * 100) if total_span_all > 0 else 0
        )
        avg_busy_percent = (
            (avg_busy / total_span_all * 100) if total_span_all > 0 else 0
        )
        ...
        f.write(
            f"Total busy time (all):       {busy_total_scaled:15.2f} {unit_label}\n"
        )
        f.write(
            f"Total idle time (all):       {idle_total_scaled:15.2f} {unit_label}\n"
        )
        f.write(
            f"Average busy time per worker:{avg_busy_scaled:15.2f} {unit_label}  [{avg_busy_percent:6.2f}%]\n"
        )
        f.write(
            f"Average idle time per worker:{avg_idle_scaled:15.2f} {unit_label}  [{avg_idle_percent:6.2f}%]\n"
        )
...
def average_timing_files(files: List[str]) -> TimingFile:
    ...
    total_runtime_ns = [
        convert_to_ns(f.total_runtime[0], f.total_runtime[1]) for f in parsed_files
    ]
    avg_runtime_ns = sum(total_runtime_ns) / n
    avg_file.total_runtime = (
        convert_from_ns(avg_runtime_ns, template.total_runtime[1]),
        template.total_runtime[1],
    )
    ...
    compute_time_ns = [
        convert_to_ns(f.total_compute_time[0], f.total_compute_time[1])
        for f in parsed_files
    ]
    avg_compute_ns = sum(compute_time_ns) / n
    avg_file.total_compute_time = (
        convert_from_ns(avg_compute_ns, template.total_compute_time[1]),
    ...
    for sys_idx in range(len(template.system_threads)):
        ...
        for task_idx in range(len(template.system_threads[sys_idx]["tasks"])):
            ...
            execs = [
                f.system_threads[sys_idx]["tasks"][task_idx]["executions"]
                for f in parsed_files
            ]
            avgs_ns = [
                convert_to_ns(
            ...
            avg_sys_task = {
                ...
                "avg": (
                    convert_from_ns(avg_avg_ns, template_task["avg"][1]),
                    template_task["avg"][1],
                ),
                "min": (
                    convert_from_ns(avg_min_ns, template_task["min"][1]),
                    template_task["min"][1],
                ),
                "max": (
                    convert_from_ns(avg_max_ns, template_task["max"][1]),
                    template_task["max"][1],
                ),
                "total": (
                    convert_from_ns(avg_total_ns, template_task["total"][1]),
                    template_task["total"][1],
                ),
            }
...
def write_timing_file(timing_file: TimingFile, output_path: str):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"Worker Slots: {timing_file.worker_slots_range}, System Thread Slots: {timing_file.system_slots_range}\n"
        )
        f.write("****************\n")
        ...
        for task in timing_file.tasks:
            f.write("  ****************\n")
            f.write(
                f"  Task '{task['name']}' - Workers: {task['workers']}, Total Executions: {task['executions']}\n"
            )
        ...
        for sys_thread in timing_file.system_threads:
            f.write(
                f"  Resolution Thread {sys_thread['thread_id']} (Slot {sys_thread['slot']}):\n"
            )
...
def main():
    ...
    parser.add_argument(
        "files", nargs="+", help="List of timing files to average (.txt and/or .csv)"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    if csv_files:
        ...
        if system_threads == 0:
            ...
            if records:
                ...
                system_threads = 1
                print(
                    f"Auto-detected {system_threads} system thread(s) from {num_slots} slots"
                )
