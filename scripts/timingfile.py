...
class TimingFile:
    ...
    def parse(self, filepath):
        ...
        while i < len(lines):
            ...
            if line.startswith("Worker Slots:"):
                match = re.search(
                    r"Worker Slots: ([\d.]+), System Thread Slots: ([\d.]+)", line
                )
                if match:
    ...
    def parse_task(self, lines, start_idx):
        ...
        match = re.search(
            r"Task '([^']+)' - Workers: (\d+), Total Executions: (\d+)", line
        )
        if not match:
            return None
            ...
            if timing_line.startswith("Timing -"):
                ...
                if avg_task_match:
                    task["timing"]["avg_task"] = (
                        float(avg_task_match.group(1)),
                        avg_task_match.group(2),
                    )
                if min_match:
                    task["timing"]["min"] = (
    ...
    def parse_system_thread(self, lines, start_idx):
        ...
        while i < len(lines):
            ...
            if (
                ...
                or task_line.startswith("Resolution Thread")
            ):
                break
