...
//! These are registered as native Τομί functions and callable from JSON

use std::net::UdpSocket;
use tomii_types::CmTypes;

/// Bind single UDP socket
/// JSON signature: tomii::bind_udp(address: String, port: usize) -> CmTypes::Any(UdpSocket)
#[no_mangle]
pub fn tomii_bind_udp(args: &[CmTypes]) -> CmTypes {
    assert_eq!(
        args.len(),
        2,
        "tomii::bind_udp(address: String, port: usize)"
    );
...
/// JSON signature: tomii::bind_udp_range(address: String, start_port: usize, count: usize)
///                 -> CmTypes::VecAny(Vec<UdpSocket>)
#[no_mangle]
pub fn tomii_bind_udp_range(args: &[CmTypes]) -> CmTypes {
    assert_eq!(
        args.len(),
        3,
        "tomii::bind_udp_range(address: String, start_port: usize, count: usize)"
    );
