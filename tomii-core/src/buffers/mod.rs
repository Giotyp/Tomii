...
//! ## The problem
//!
//! Τομί runs many frames through the same slot in sequence.  Each slot
//! owns a set of [`NodeDependencyEntry`] counters and sent-flags that must be
//! reset to their initial values at the start of every new frame.  A naive
//! O(nodes × factor) sweep before each frame is both slow and racy: worker
//! threads completing the tail of frame N can still be decrementing counters
//! while the reset for frame N+1 is in progress.
//!
...
//! ```
//!
//! - **Upper 32 bits**: a generation counter, incremented once per frame in
...
//! effectively resets the counter for that frame.
//!
...
//! then discards the task without decrementing the new frame's counters.
