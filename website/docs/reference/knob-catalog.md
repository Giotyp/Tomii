...
This page transcribes catalog version 2. Each knob carries a name (the
...
| `max_runtime` | `--max-runtime` | int | measurement | — | Stop after N seconds (0 = run until max_streams complete) |
| `slots` | `--slots` | int | perf | 1–64, pow2 | Concurrent in-flight streams (1 for latency, &gt;1 for throughput) |
| `max_streams` | `--max-streams` | int | measurement | — | Total streams to process (0 = run indefinitely) |
...
| `exclude_streams` | `--exclude-streams` | int | measurement | — | Skip first N streams from timing output |
| `record_stream` | `--record-stream` | int | measurement | — | Record timing for this specific stream index only |
...
| `inits` | `--inits` | bool | measurement | bool | Re-run graph initializations on each stream |
