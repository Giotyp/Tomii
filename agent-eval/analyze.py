...
def bootstrap_ci(
    values: list[float], n_boot: int = 2000, ci: float = 0.95
) -> tuple[float, float]:
    n = len(values)
    if n == 0:
...
def log_rank_test(
    group_a: list[dict], group_b: list[dict], cap: int = 50_000_000
) -> float | None:
...
def print_primary_table(
    groups: dict[str, list[dict]], q_by_cond: dict[str, float]
) -> None:
    ...
    for cond in cond_order:
        ...
        n = len(ts)
        pass_rate = (
            100 * sum(1 for t in ts if t.get("verify_pass") or t.get("passed")) / n
        )
        q_rate = 100 * sum(1 for t in ts if t.get("reached_q")) / n
        tok_q = [
            _enr(t, "tokens_to_q") for t in ts if _enr(t, "tokens_to_q") is not None
        ]
        med = _median(tok_q)
        lo, hi = bootstrap_ci(tok_q) if tok_q else (None, None)
        ci_str = f"[{lo:>7.0f}, {hi:>7.0f}]" if lo is not None else "—"
        print(
            f"{cond:<8} {n:>3} {pass_rate:>5.1f}% {q_rate:>4.1f}%"
...
def print_interface_effect(groups: dict[str, list[dict]]) -> None:
    ...
    for key, label in metrics:
        ...
        if vf is not None and vb is not None and vb != 0:
            ratio = vf / vb
            print(
                f"  {label:<25}: T-full={_fmt(vf, 1)} / T-bare={_fmt(vb, 1)} (ratio {ratio:.2f}×)"
            )
        else:
    ...
    if args.output:
        ...
        for cond, ts in groups.items():
            ...
            summary[cond] = {
                "n": len(ts),
                "pass_rate": sum(
                    1 for t in ts if t.get("verify_pass") or t.get("passed")
                )
                / len(ts),
