...
def build_graph() -> tm.Graph:
    ...
    generate = app.node(
        "generate", func="generate_reading",
        factor=total_readings, args=[anomaly_threshold],
    )

    classify = app.node(
        "classify", func="classify_reading",
        factor=total_readings,
        args=[generate.out(), anomaly_threshold],
    )

    cond_anomaly = tm.Condition(
        operation="Eq", value=True, value_type="bool",
        func="check_bool", args=[classify.out()],
    )
    app.node(
        "handle_anomaly", func="amplify_reading",
        factor=total_readings, priority="high", condition=cond_anomaly,
        args=[generate.out(), classify.wait()],
    )

    cond_normal = tm.Condition(
        operation="Neq", value=True, value_type="bool",
        func="check_bool", args=[classify.out()],
    )
    app.node(
        "smooth", func="smooth_reading",
        factor=total_readings, priority="low", condition=cond_normal,
        args=[generate.out(), classify.wait()],
    )

    compute_stats = app.node(
        "compute_stats", func="compute_sensor_stats",
        factor=num_sensors,
        args=[generate.wait(0, total_readings - 1, group_by=readings_per_sensor)],
    )

    app.node(
        "log_event", func="log_stream_event",
        args=[classify.dep(0, total_readings - 1)],
    )

    aggregate = app.node(
        "aggregate", func="aggregate_results",
        factor=num_sensors,
        args=[
            compute_stats.out(),
