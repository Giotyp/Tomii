"""Generate Python tomii API code from a GraphFile-compatible dict."""
...
def generate_python(graph_data: dict) -> str:
    ...
    ----------
    graph_data:
        A dict matching the GraphFile JSON schema (same format as Τομί JSON files).

    Returns
    -------
    str
        Valid Python source that imports tomii and defines ``build_graph() -> tm.Graph``.
    """
    lines: list[str] = []
    lines.append("import tomii as tm")
    lines.append("from tomii import Condition, Loop")
    lines.append("")
    lines.append("")
    lines.append("def build_graph() -> tm.Graph:")
    lines.append("    app = tm.Graph()")
    lines.append("")
...
def _parse_init_value(arg: dict) -> str:
    ...
    if type_.startswith("Vec"):
        return f"tm.Vec({type_[4:-1]!r}, [{value}])"
