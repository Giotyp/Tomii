"""Build orchestration: compile synstream-core + plugin library."""
...
class BuildResult:
    ...
    binary: str  # Absolute path to synstream binary
...
def find_workspace_root() -> Path:
    ...
    raise BuildError(
        ...
        "Ensure the synstream package is inside the SynStream workspace."
    )
...
def _cargo(args: list, env: dict, cwd: Path) -> None:
    ...
    cmd = ["cargo"] + args
    print(f"[synstream.build] {' '.join(cmd)}", flush=True)
...
def build(config: BuildConfig) -> BuildResult:
    """Run the two-stage SynStream build and return paths to outputs."""
    ...
    # Build the synstream main binary last so the plugin clean can't wipe it.
    # (cargo clean --manifest-path on a workspace member cleans the whole workspace.)
    if config.clean:
        _cargo(["clean", "-p", "synstream-core"], build_env, workspace)

    _cargo(["build", "-p", "synstream-core", "--bin", "main"] + release_flag, build_env, workspace)
    _cargo(["build", "-p", "synstream-types"] + release_flag, build_env, workspace)
