/// Taskflow bilateral image denoising benchmark.
///
/// Implements a 2D wavefront DAG where each node T(i,j) applies a bilateral
/// filter to one tile of the image.  T(i,j) depends on T(i-1,j) and T(i,j-1)
/// so that halo pixels written by predecessors are visible.
///
/// Usage:
///   ./bilateral_wavefront \
///     --image-size 4096 --tile-size 256 --kernel-radius 4 \
///     --sigma-s 3.0 --sigma-r 0.1 \
///     --threads 8 --iterations 10 \
...
// ---------------------------------------------------------------------------
// npy loader (float32, 2-D, C-order)
// ---------------------------------------------------------------------------
...
static std::vector<float> load_npy_f32(const std::string& path, int& H, int& W) {
    ...
    // Parse shape=(H, W) from header dict string
    auto find_int = [&](const std::string& after) -> int {
        size_t pos = hdr.find(after);
        if (pos == std::string::npos)
            throw std::runtime_error("Header parse error looking for: " + after);
        pos += after.size();
        while (pos < hdr.size() && !std::isdigit(hdr[pos])) ++pos;
        int v = 0;
        while (pos < hdr.size() && std::isdigit(hdr[pos]))
            v = v * 10 + (hdr[pos++] - '0');
        return v;
    };
    ...
    // extract first two integers after '('
...
struct BenchmarkState {
    ...
    float sigma_r    = 0.1f;
    int kernel_radius = 4;
    double psnr_result = 0.0;
};

// ---------------------------------------------------------------------------
// Bilateral filter — one tile
// ---------------------------------------------------------------------------
...
struct Cli {
    ...
    std::string output   = "tf_bilateral.csv";
};
...
static void pin_workers(tf::Executor& executor, int base_core) {
    ...
    for (size_t i = 0; i < nw; ++i) {
        ...
        auto h = executor.async([cs]() mutable {
...
int main(int argc, char** argv) {
    ...
    const int N = S / T;   // grid_n

    std::printf("Taskflow Bilateral | image=%dx%d tile=%d kernel_r=%d sigma_s=%.1f sigma_r=%.2f threads=%d\n",
    ...
    tf::Executor executor(cli.threads);
    if (cli.pin) pin_workers(executor, 1);

    // Build wavefront DAG (once — reused across iterations)
    // grid[i][j] is a tf::Task that runs bilateral_filter_tile(st, i, j)
    std::vector<std::vector<tf::Task>> tiles(N, std::vector<tf::Task>(N));
    tf::Taskflow taskflow;

    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            tiles[i][j] = taskflow.emplace([&st, i, j] {
                bilateral_filter_tile(st, i, j);
            }).name("t_" + std::to_string(i) + "_" + std::to_string(j));

            if (i > 0) tiles[i-1][j].precede(tiles[i][j]);
            if (j > 0) tiles[i][j-1].precede(tiles[i][j]);
        }
    }

    // Each run overwrites every pixel exactly once — no need to zero first.
    auto run_once = [&] {
    ...
    const std::string label = cli.pin ? "taskflow_pinned" : "taskflow";
    std::printf("%s | image=%d tile=%d kr=%d threads=%d | mean=%.2f ms | PSNR=%.2f dB\n",
                label.c_str(), S, T, cli.kernel_radius, cli.threads, mean_ms, psnr);

    append_csv(cli.output, label, S, T, cli.kernel_radius, cli.threads, mean_ms, psnr, N);
    return 0;
}
