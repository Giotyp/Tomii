...
mod tests {
    ...
    #[test]
    fn test_extract_frame_id_u8() {
        let packet = vec![0xAA, 0xBB, 0xCC, 0xDD, 0x42, 0xFF];
        assert_eq!(extract_frame_id_fixed(&packet, 4, 1), 0x42);
    }

    #[test]
    fn test_extract_frame_id_u16_le() {
        let packet = vec![0xAA, 0xBB, 0xCC, 0x34, 0x12, 0xFF];
        assert_eq!(extract_frame_id_fixed(&packet, 3, 2), 0x1234);
    }

    #[test]
    fn test_extract_frame_id_u32_le() {
        let packet = vec![0xAA, 0xBB, 0x78, 0x56, 0x34, 0x12, 0xFF];
        assert_eq!(extract_frame_id_fixed(&packet, 2, 4), 0x12345678);
    }

    #[test]
    fn test_extract_frame_id_u64_le() {
        let packet = vec![0xAA, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0xFF];
        assert_eq!(
            extract_frame_id_fixed(&packet, 1, 8),
            0x1122334455667788usize
        );
    }

    #[test]
    #[should_panic(expected = "Unsupported frame ID length")]
    fn test_extract_frame_id_invalid_length() {
        let packet = vec![0xAA, 0xBB, 0xCC, 0xDD];
        extract_frame_id_fixed(&packet, 0, 3);
    }

    #[test]
    #[should_panic(expected = "> packet size")]
    fn test_extract_frame_id_out_of_bounds() {
        let packet = vec![0xAA, 0xBB, 0xCC, 0xDD];
        extract_frame_id_fixed(&packet, 2, 4);
    }
}
