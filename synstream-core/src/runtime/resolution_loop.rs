...
impl super::SynRt {
    ...
    pub(super) fn resolution(
        ...
        loop {
                    ...
                    for packet_msg in packet_buf.drain(..) {
                        ...
                        if let CmTypes::Bytes(arc) = received_bytes_cm {
                            if let Ok(buf) = Arc::try_unwrap(arc) {
                                if let Some(tx) = shared.net.buffer_return_senders.get(packet_socket_id)
                                {
                        ...
                        if let Some(new_stream) = new_stream_opt {
                            ...
                            let (assigned_slot, newly_activated) = match assign_stream_to_available_slot(&shared, new_stream) {
                                ...
                                None => {
                                    ...
                                    if new_stream < shared.net.frame_dropped.len() {
                                        let already_marked = shared.net.frame_dropped[new_stream]
                                            .swap(true, Ordering::AcqRel);
                                        if !already_marked {
                                            shared.telemetry.stream_complete_counter
                                                .fetch_add(1, Ordering::SeqCst);
                                            let dropped =
                                                shared.net.dropped_streams.fetch_add(1, Ordering::Relaxed)
                                                    + 1;
                                            eprintln!(
                            ...
                            let packet_index = if let Some((idx_fn, idx_args)) = idx_func_ptr {
                                ...
                                shared.slot_data.packet_counters[node_info.slot]
                                    .fetch_add(1, Ordering::SeqCst);
                                idx_result
                        ...
                        let slot_is_active = shared.slot_data.active_bitmap.load(Ordering::Acquire)
                        ...
                        let packet_count =
                            shared.slot_data.packet_counters[node_info.slot].load(Ordering::SeqCst);
                        if packet_count == stream_packets {
                            // Exactly-once semantics: atomically claim completion ownership
                            let already_completed = shared.slot_data.packet_complete[node_info.slot]
                                .swap(true, Ordering::SeqCst);

                            if !already_completed {
                                // DEBUG: Check counter values when all packets received
                                let pending_tasks = shared.slot_data.pending_tasks[node_info.slot]
                                    .load(Ordering::SeqCst);
                                let pending_cond = shared.slot_data.pending_cond_tasks[node_info.slot]
                                    .load(Ordering::SeqCst);
                                print_debug(|| {
                                    format!(
            ...
            batch_buf.clear();
            batch_buf.extend(shared.exec.batch_queue_rx.try_iter().take(shared.config.target_batch_size));
            if batch_buf.is_empty() {
                ...
                for _ in 0..shared.config.spin_iterations {
                    ...
                    if let Ok(item) = shared.exec.batch_queue_rx.try_recv() {
                        batch_buf.push(item);
                        batch_buf.extend(
                            shared.exec.batch_queue_rx.try_iter().take(shared.config.target_batch_size - 1),
                        );
                        break;
                    }
                }
                ...
                TASK_COMP_BUF.with(|tbuf| {
                    ...
                    comp_batch.extend(batch_buf.drain(..).filter(|n| {
                        if gen_loaded & (1u64 << n.slot) == 0 {
                            gen_cache[n.slot] = shared.slot_data.generation[n.slot].load(Ordering::SeqCst) as u32;
