...
impl TimeBuffer {
    ...
    pub fn write_json_report(
        ...
        // (task_name -> Vec<(worker_id, duration)>) per stream
        ...
        // Determine num_workers for pct_of_total denominator.
        let num_workers = {
            let max_w = worker_busy_us.keys().copied().max().unwrap_or(0);
            // A rough count: highest worker id + 1 (or at least 1)
            max_w + 1
        };
        ...
        let critical_path: Option<CriticalPath> = if !graph_edges.is_empty() {
            ...
            // DP: dist[i] = max latency path ending at node i
        ...
        // Denominator: avg_latency_us * num_included_streams (total elapsed time per worker)
        ...
        // (< 8), indicating the graph may be over-coarsened — too few tasks per CP node to
        // saturate workers. Recommends increasing factor to expose more parallelism.
        // This prevents agents from coarsening further (wrong direction) when no other
        // suggestion fires.
        ...
        let mut per_node_entries: Vec<serde_json::Value> = node_stats_map
            ...
            .collect();
        // Sort by total_exec_us descending for readability
